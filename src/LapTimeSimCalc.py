...
class LapTimeSimCalc:
        ...
        def Run(self):
            ...
            for i in reversed(range(len(dist))):
                axdecmap[i] = np.interp(vxdec[i],self.vxvect,self.axdec,period=360)
                aymap[i] = max(np.interp(vxacc[i],self.vxvect,self.ay,period=360),small)
                ayreal[i] = pow(vxdec[i],2)/(1/max(curv[i],small))
                axcombine[i] = axdecmap[i]*mt.sqrt(np.absolute(1-(ayreal[i]/aymap[i])))
                vxdec[i-1] = min(vxcor[i-1],(vxdec[i]+(dist[i-1]-dist[i])/vxdec[i]*axcombine[i]))
            vxdec[-1] = vxacc[-1]
